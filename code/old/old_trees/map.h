...
public:
  ...
  static sequence<OT> to_seq(M m, F f) {
  ...
  // insert multiple keys from an array
  //template<class Seq>
  static M multi_insert_sorted(M m, sequence<E> SS, bool seq_inplace = false, bool run_seq=false) {
    auto replace = [] (const V& a, const V& b) {return b;};
  ...
  static M multi_insert_reduce(M m, sequence<pair<K,Val>> S, Reduce g, bool sorted=false) {
    auto replace = [] (const V& a, const V& b) {return b;};
	timer t,t1;
	t.start();
  sequence<E> A = Build::sort_reduce_duplicates(S, g, sorted);
...
//	std::cout << "time multi_insert: " << t1.stop() << std::endl;
  ...
  // F: (entry, i) -> ()
  template <class F>
  void map_elms(F f) const {
    Tree::foreach_index_bc(root, 0, f);
  }

  // F: (entry) -> ()
  template <class F>
  void map_elms_no_index(F f) const {
