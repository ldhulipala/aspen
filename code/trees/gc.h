...
struct gc {
  ...
  static void init(size_t n=default_alloc_size, size_t ls=list_size) { alloc::init(n, ls);}
  ...
  static bool decrement_recursive(node* t, bool run_seq=false) {
    if (!t) return false;
    node* lsub = t->lc;
    node* rsub = t->rc;
    if (decrement(t)) {
      utils::fork_no_result((Node::size(lsub) >= utils::node_limit) && !run_seq,
  ...
  static inline node* copy(node* t) {
    node* o = Node::make_node(Node::copy_entry(t));
    o->lc = inc(t -> lc);
    o->rc = inc(t -> rc);
    return o;
  }
  ...
  static inline node* copy_if(node* t, bool copy, bool extra_ptr) {
    if (copy) {
      node* r = Node::make_node(Node::copy_entry(t));
...
template <typename E>
  ...
  static void used_bytes() {
    if (!alloc::initialized)
      std::cout << "allocator not initialized" << std::endl;
    else {
      size_t used = alloc::num_used_blocks();
      size_t size = alloc::block_size();
      size_t used_bytes = used*size;
      std::cout << "used: " << used_bytes << std::endl;
    }
  }
