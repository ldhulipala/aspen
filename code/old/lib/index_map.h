...
struct out_imap {
  ...
  out_imap<T,F> slice(size_t ss, size_t ee) {return out_imap<T,F>(s+ss,s+ee,f); }
...
public:
  ...
  array_imap& operator = (const array_imap& m) {
    if (this != &m) {free_mem(); s = m.s; e = m.e; allocated=false;}
    return *this;
  }
  // move assignment
  array_imap& operator = (array_imap&& m) {
    if (this != &m) {s = m.s; e = m.e; allocated=m.allocated; m.allocated=false;}
    return *this;
  }
  void del() {
    if (allocated) {
  ...
  const size_t size() { return e - s;}
  T* get_array() {allocated = false; return s;}
  T* start() {return s;}
  T* end() {return e;}
